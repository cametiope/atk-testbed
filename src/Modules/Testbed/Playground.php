...
class Playground extends Node {
	public function __construct( $nodeUri ) {
		parent::__construct( $nodeUri, Node::NF_MULTI_RECORD_ACTIONS ); // Node::NF_ADD_LINK |
	...
	protected function addDefaultFields( $tab ) {
		...
		$this->add( new ListAttribute( 'theListAttribute', A::AF_SEARCHABLE, [ 'testo lungo della option_4', 'option_5', 'testo lungo della option_6' ],
			[ 4, 5, 6 ] ),
			$tab );
		$this->add( new ManyToOneRelation( 'FM2O', A::AF_LARGE | A::AF_SEARCHABLE, $this->getModule() . '.m2o_node' ), $tab );
		$this->add( new PasswordAttribute( 'PasswordAttribute', PasswordAttribute::AF_PASSWORD_NO_VALIDATE, true,
		...
		$this->add( new ShuttleRelation( 'ShuttleRelation', 0, $this->getModule() . '.m2m_node', $this->getModule() . '.m2o_node', 'playground_id',
			'remotetable_id' ),
			$tab );
	}
	...
	protected function testHideSHow() {
		...
		$this->add( new DummyAttribute( 'DummyAttribute', DummyAttribute::AF_DUMMY_SHOW_LABEL,
			'Testo del Dummy Attribute' ) )->setHelp( 'help dummy attribute, ciao ciao!' );
	}
...
//        $this->add(new OneToManyRelation('the1OneToManyRelation', A::AF_SEARCHABLE, $this->getModule().'.o2m_node', 'playground_id'), $tab);
	...
	protected function testSearch() {
		...
		$attr = new ManyToOneRelation( 'FM2O', ManyToOneRelation::AF_MANYTOONE_OBLIGATORY_NULL_ITEM | A::AF_SEARCHABLE | A::AF_FORCE_LOAD,
